...
/*
 *	Baud rate selection added in Version 5.4:
 *
 *		It seems the software has a problem working with the N1MM Rotor program which can
 *		only run at 9600 baud, It seems to work fine with the SatPC32 rotator interface
 *		(ServerSDX) running at 57600 baud.
 */

// 	#define BAUD_RATE	57600				// For SatPC32
 	#define	BAUD_RATE	 9600				// For N1MM Rotor


/*
 *	The "ELEVATION" symbol is added in Version 5.4X. If set to "true" hardware serial 
 *	port #2 is used to communicate with a second processor responsible for providing
 *	elevation control for the antenna.
 *
 *	This is purely experimental at the moment as there are a lot of details to be
 *	worked out once we can determine what commands the SatPC32 program uses to send
 *	elevation commands (there are a number of options)! The baudrate for the second
 *	serial port will be the same as for the primary (USB) port.
 */
 ...
 *	in microseconds.
 *	
 *	The rotator I used for testing and development took about 50 seconds to complete
 *	a 360 degree rotation. Pulses from the cam operated switch were seen an average of
 *	every 784Ms. The value of "timerInterval" should be set to about 4 - 5% longer than
 ...
 *	
 *	The value that seems to work for both of my rotators was determined to be 140Ms.
 */

	volatile uint32_t	timerInterval = 140000;		// Generate interrupts every 137mS
 ...
 *	
 *	Again, my rotator sent cam switch pulses approximately every 784Ms, so the setting of
 *	the "rotatorTimeout" is 850 milliseconds.
 *
 *	"INITIAL_TIMOUUT" is added in Version 5.4. We use it at the initial motor start up, and
 *	after we have seen 2 hard pulses, switch to the normal timeout. This cures two problems:
 *
 *		1.	With a big antenna, there is some startup inertia that must be compensated for.
 *		2.	If the rotator happened to stop with the cam switch operated, it was immediately
 *			timing out.
 */

	#define	PULSE_DEBOUNCE	  15L				// Debounce limit = 15 mS
	#define	ROTATOR_TIMEOUT	 850L				// Timeout between cam pulses
	#define	INITIAL_TIMEOUT	2000L				// Extra delay on motor startup for inertia
...
//	define	LCD_ADDRESS 0x27			// Mike's controller
	#define	LCD_ADDRESS 0x3F			// My controller
...
/*
 *	If "AZ_SW_INSTALLED" is set to "true", the 3 position toggle switch that allows
 *	the operator to select the number of degrees to change the target azimuth for each
 *	encoder click is installed. It the switch has not been installed, the definition
 *	should be set to "false" or weitd things will happen!
 *
 ...
 */
	...
	#define AZ_SW_INSTALLED	true			// I have the switch installed
